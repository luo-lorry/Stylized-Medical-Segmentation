...
def train(train_loader, model, optimizer, epoch, opt, total_step):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            loss = 1 * loss_output + 0.5 * loss_attention  # 可以调整权重，例如 loss = loss_output + 0.5 * loss_attention
            # loss = 1.5 * loss_output + 0.5 * loss_attention
            # ---- backward ----
            loss.backward()
    ...
    # ---- save model ----
    save_path = 'snapshots-Skin/{}/'.format(opt.train_save)
    os.makedirs(save_path, exist_ok=True)
    if (epoch + 1) % 10 == 0:
        torch.save(model.state_dict(), save_path + 'UNet-%d.pth' % epoch)
        print('[Saving Snapshot:]', save_path + 'UNet-%d.pth' % epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_save', type=str,
                        default='UNet(style_transfer)', help='path to save trained models')
    opt = parser.parse_args()

    # ---- build models ----
    model = UNet(in_channels=3, out_channels=1, base_channels=32, pretrained=True).cuda()
    ...
    # ---- data loader ----
    image_root = './Dataset/Dataset_Skin/train/style(ST)/'
    gt_root = './Dataset/Dataset_Skin/train/masks/'
